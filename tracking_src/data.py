...
def open_video(simulation_path: pathlib.Path) -> byotrack.Video:
    ...
    video.set_transform(
        byotrack.VideoTransformConfig(aggregate=True, normalize=True, selected_channel=0, q_min=0.0, q_max=1.0)
    )
    return video
...
def load_ground_truth(simulation_path: pathlib.Path) -> Dict[str, torch.Tensor]:
    ...
    return torch.load(simulation_path / "video_data.pt", weights_only=True)
